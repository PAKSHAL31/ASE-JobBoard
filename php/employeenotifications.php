...
<body>
  ...
  <?php
$sql = "SELECT j.job_position, j.job_location, j.job_type, j.company_no, ej.ar_val, ej.comment 
...
if ($numrows == 0) {
  echo '<h4 style="color:black;">No Notifications</h4>';
}
...
while ($row = mysqli_fetch_assoc($result)) {
  ...
  $company = mysqli_fetch_assoc($ans);
  ?>
  <div class="component">
    <div class="job">
      <h3><?php echo $row['job_position']; ?></h3>
      <h4 class="job-type"><?php echo $row['job_type']; ?></h4>
    </div>
    
    <div class="company-location">
      <a class="locate"><i class="fa fa-server" aria-hidden="true"></i>
        <span><?php echo $company['company_name']; ?></span></a>
    ...
    <div class="application">
      <?php
      if ($row['ar_val'] == 1) {
        echo '<span class="toggle-accepted">Accepted</span>';
      } else {
        echo '<span class="toggle-rejected">Rejected</span>';
      }
      ?>
    </div>

    <!-- Button to view comments -->
    <button class="comment-btn" onclick="toggleComments('<?php echo $row['job_position']; ?>')">View Comments</button>
    
    <!-- Hidden comments text block -->
<div class="comments-text" id="comments-text-<?php echo $row['job_position']; ?>" style="display: none;">
  <!-- Close button placed to the right of the comments -->
  <button class="close-btn" onclick="toggleComments('<?php echo $row['job_position']; ?>')">Close</button>
  <p><?php echo $row['comment'] ?: 'No comments available.'; ?></p>
</div>
